...
## Memorial Project — Infra Context

### What This Repo Is

Infrastructure and local dev environment for the Memorial NFC photo gallery platform.
Postgres + MinIO + Mailhog via Docker Compose. Phase 2 = cloud (Neon/Supabase + Cloudflare R2 + Vercel + Railway).

### Local Dev Stack

| Service        | Port | Credentials                           |
| -------------- | ---- | ------------------------------------- |
| PostgreSQL     | 5432 | `memorial:memorial` / db: `memorial`  |
| MinIO API      | 9000 | `memorial:memorial123`                |
| MinIO Console  | 9001 | `memorial:memorial123`                |
| Mailhog UI     | 8025 | —                                     |

```bash
./scripts/dev-up.sh    # start all services
./scripts/dev-down.sh  # stop
./scripts/dev-reset.sh # nuke volumes + fresh start
```

Compose file: `docker/docker-compose.yml`
DB schema: `db/init.sql` (auto-applied on first postgres start)
Project brain: `context.md` — update when making structural changes

### Autonomy

Full autonomy on local infra changes. Pause for: running production changes, destructive data ops, external cloud resource changes.

### Security

- NEVER commit `.env` files or credentials
- MinIO `memorial` bucket: anonymous download enabled for dev only
- Production: signed URLs, no public bucket
